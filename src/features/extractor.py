"""
    ...
    Classes
    -------
    FeatureVector   : thin wrapper (values + names)
    FeatureExtractor: computes per-sentence feature vectors;
                      collectFeatures(document) → FeatureVector is the primary API.
"""
...
# Function words taken from the top-200 of the Google 10 k English list
...
class FeatureVector:
    """
    Wraps the raw float array together with feature names.
...
class FeatureExtractor:
    ...
    collectFeatures(document) -> FeatureVector
        Accepts a Document object (from data.loader) or any object with a
        `.sentences` attribute, and returns a FeatureVector whose `.values`
        has shape [n_sentences, numFeatures].
    """
    ...
    def collectFeatures(self, document) -> FeatureVector:
        """
        Extract features for every sentence in *document*.

        Parameters
        ----------
        document : Document (data.loader.Document) or any object with
                   a `.sentences: List[str]` attribute.

        Returns
        -------
        FeatureVector
            .values has shape [n_sentences, numFeatures]
            .names  is a copy of featureNames
        """
        sentences = document.sentences
        matrix    = self.transform_document(sentences)
        return FeatureVector(values=matrix, names=list(self.featureNames))

    # ── per-feature helpers (one method per logical group) ────────

    def _basic_counts(self, sentence: str):
        """Group 1 — character and word counts, punctuation."""
        alpha   = re.findall(r"[a-zA-Z]+", sentence)
        n_alpha = max(1, len(alpha))
...
if __name__ == "__main__":
    ...
    # collectFeatures demo using a mock document
    class _MockDoc:
        sentences = [
